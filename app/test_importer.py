...
from database import initialize_database, display_all_data, delete_item_by_id, add_product
...
class TestImporter(unittest.TestCase):
    ...
    def test_import_csv_files(self):
        ...
        test_data = "name,quantity,price,category\nApple,10,1.2,Fruit\nBanana,20,0.8,Fruit"
        with open("test.csv", "w") as f:
            f.write(test_data)

        # Passer la connexion à la base de données à la fonction import_csv_files
    ...
    def test_initialize_database(self):
        ...
        # Remplacer la connexion en mémoire par un chemin de fichier pour l'initialisation de la DB
    ...
    def test_delete_item_by_id(self):
        ...
        self.assertIsNone(result_after_delete)  # Le produit ne doit plus exister
    ...
    def test_display_all_data(self):
        ...
        with patch('sys.stdout', new_callable=io.StringIO) as captured_output:
            ...
            self.assertIn("Apple", output)  # Vérifier que "Apple" est dans l'affichage
            self.assertIn("Banana", output)  # Vérifier que "Banana" est dans l'affichage
...
class TestReport(unittest.TestCase):
    ...
    def test_generate_summary_report_with_data(self):
        ...
        # Vérifier que le fichier "test_file.csv" existe bien après la génération
        ...
        with open("test_file.csv", "r") as f:
            ...
            self.assertEqual(rows[0], ["Category", "Number of Products", "Total Quantity", "Total Value"])
    ...
    def test_generate_summary_report_with_no_data(self):
        """Test de la génération du rapport lorsque la base de données est vide"""
        # Appeler la fonction qui génère le rapport (aucune donnée dans la table)
        generate_summary_report(self.conn, "test_file.csv")
        ...
        with open("test_file.csv", "r") as f:
            ...
            self.assertEqual(rows, [["Category", "Number of Products", "Total Quantity", "Total Value"]])

    def test_generate_summary_report_empty_categories(self):
        """Test de la génération du rapport si certaines catégories sont vides"""
...
class TestSearchProducts(unittest.TestCase):
    ...
    def test_search_products_with_single_criteria(self):
        ...
        # Appeler la fonction avec un critère de recherche (par exemple, "name:Apple")
    ...
    def test_search_products_with_multiple_criteria(self):
        ...
        self.assertEqual(len(all_products), 4, "Les produits n'ont pas été correctement insérés.")

        # Appeler la fonction avec plusieurs critères de recherche (par exemple, "name:Apple" et "category:Fruit")
    ...
    def test_search_products_with_no_results(self):
        """Test de recherche avec des critères ne correspondant à aucun produit"""
        # Ajouter des produits à la base de données
        self.cursor.executemany('''
            INSERT INTO inventory (name, quantity, price, category)
            VALUES (?, ?, ?, ?)
        ...
        # Appeler la fonction avec des critères qui ne correspondent à aucun produit
        ...
        with patch('sys.stdout', new_callable=io.StringIO) as captured_output:
            ...
            output = captured_output.getvalue().strip()
            self.assertEqual(output, "")  # Aucun résultat trouvé, donc sortie vide
    ...
    def test_search_products_with_wildcards(self):
        ...
        # Appeler la fonction avec un critère incluant un joker, par exemple "name:Ap"
    ...
    def test_search_products_empty_criteria(self):
        ...
        with patch('sys.stdout', new_callable=io.StringIO) as captured_output:
            ...
            # Vérifier que tous les produits sont trouvés (aucun critère de recherche)
            output = captured_output.getvalue().strip()
            self.assertIn("Apple", output)
